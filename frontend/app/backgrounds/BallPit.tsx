...
// Global variable to store loaded drone model
let loadedDroneGeometry: BufferGeometry | null = null;
let loadedDroneScene: THREE.Group | null = null;
let isLoadingModel = false;

// Try to load GLTF drone model
function loadDroneModel(): Promise<BufferGeometry> {
  return new Promise((resolve) => {
    if (loadedDroneGeometry) {
      resolve(loadedDroneGeometry);
      return;
    }
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        if (geometries.length > 0) {
          loadedDroneGeometry = mergeGeometries(geometries);
          loadedDroneGeometry.scale(6, 6, 6);
          loadedDroneGeometry.center();
          console.log('✅ Loaded drone model from /models/drone.gltf');
        }
        resolve(loadedDroneGeometry || createProceduralDroneGeometry());
      },
      undefined,
      (error) => {
        console.log('ℹ️ No drone model found, using procedural geometry');
        resolve(createProceduralDroneGeometry());
      }
    );
  });
}

// Create procedural drone geometry (fallback)
function createProceduralDroneGeometry(): BufferGeometry {
  const geometries: BufferGeometry[] = [];
  
  // Main body (center box) - compact size
  const bodyGeometry = new BoxGeometry(0.4, 0.15, 0.4);
  geometries.push(bodyGeometry);
  
  // Arms (4 diagonal arms) - thin and precise
  const positions = [
    { x: 0.25, z: 0.25, rotation: Math.PI / 4 },
    { x: -0.25, z: 0.25, rotation: -Math.PI / 4 },
    { x: 0.25, z: -0.25, rotation: -Math.PI / 4 },
    { x: -0.25, z: -0.25, rotation: Math.PI / 4 }
  ];
  ...
  positions.forEach(pos => {
    // Arm - thin
    const armGeometry = new BoxGeometry(0.5, 0.06, 0.06);
    armGeometry.rotateY(pos.rotation);
    armGeometry.translate(pos.x, 0, pos.z);
    geometries.push(armGeometry);
    
    // Propeller at end of arm - small
    const propGeometry = new CylinderGeometry(0.18, 0.18, 0.04, 8);
    propGeometry.translate(pos.x * 1.8, 0.12, pos.z * 1.8);
  ...
  geometries.forEach(geo => {
    ...
    if (geo.index) {
      ...
      for (let i = 0; i < positions.length / 3; i++) {
        mergedIndices[indexOffset + i] = vertexCount + i;
      }
...
class Z extends InstancedMesh {
  ...
  constructor(renderer: WebGLRenderer, params: Partial<typeof XConfig> = {}) {
    ...
    // Start with procedural geometry
    const geometry = createProceduralDroneGeometry();
    ...
    // Try to load GLTF model asynchronously and replace geometry
    loadDroneModel().then((loadedGeometry) => {
      if (loadedGeometry && loadedGeometry !== geometry) {
  ...
  update(deltaInfo: { delta: number }) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      // Add realistic tilting based on velocity
      const velocityIdx = 3 * idx;
      const vx = this.physics.velocityData[velocityIdx];
      const vz = this.physics.velocityData[velocityIdx + 2];
      
      // Tilt forward/backward based on forward velocity
      U.rotation.x = -vz * 3;
      // Tilt left/right based on sideways velocity
      U.rotation.z = vx * 3;
      // Gentle rotation for variety (not spinning propellers)
      U.rotation.y = Math.sin(time * 0.5 + idx) * 0.3;
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  ...
  threeInstance.renderer.toneMappingExposure = 1.2;
  threeInstance.maxPixelRatio = 2;
  threeInstance.minPixelRatio = 1.5;
